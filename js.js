...
Object.entries(itemBuilds).forEach(
    ([itemName, data]) => {
        data.buildsFrom.forEach(componentName => {
            if (!buildsInto[componentName]) { // Lägg bara till itemet om det inte redan finns i listan
                buildsInto[componentName] = [];
            } 
            if (!buildsInto[componentName].includes(itemName)) {
                buildsInto[componentName].push(itemName);
            }
        });
    }
);
...
buildBox.addEventListener(
    "dragover",
    event => {

        event.preventDefault();

        buildBox.classList.add(
            "drag-over"
        );
...
buildBox.addEventListener("drop", event => {
    ...
    // ==================================================
    // ITEM SOM REDAN FINNS I BUILDEN
    // ==================================================
...
// ==================================================
// BESTÄM VAR ITEMET SKA HAMNA VID DROP
// ==================================================
...
// ==================================================
// BESTÄM VAR EDIT-ITEM SKA HAMNA
// ==================================================
function getEditDropTarget(
    container,
    x
) {
    ...
    const draggableElements = [
        ...container.querySelectorAll(
            ".edit-build-item:not(.being-dragged)"
        )
    ];
...
function addItemToBuild(item) {
    ...
    buildItem.innerHTML = `
        ...
        <button
            class="remove-build-item"
            type="button"
            title="Remove build item"
        >
            ×
        </button>
...
function showItemInfo(item) {
    ...
    overlay
        .querySelectorAll(
            ".modal-image, .modal-component, .modal-builds-into-item"
        )
        .forEach(element => {
        ...
        // ==================================================
        // DRA ITEM FRÅN MODAL TILL BUILD
        // ==================================================
                ...
                element.addEventListener(
                    ...
                    event => {
                       ...
                       event.dataTransfer.setData(
                        ...
                        // Lägg buildboxen ovanpå overlayn
                        // utan att flytta den
...
function showSaveBuildModal() {
    ...
    if (currentItems.length === 0) {
        ...
        alert(
            "You need to add atleast one item before you can save the build."
        );
    ...
    overlay.querySelector(
        ...
        () => {
            ...
            if (!name) {
                ...
                alert(
                    "You need to give the build a name."
                );
            ...
            if (existingBuild) {
                ...
                const overwrite =
                    confirm(
                        `There is already a build with that name "${existingBuild.name}". Would you like to replace it?`
                    );
